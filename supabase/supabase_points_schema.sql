...
BEGIN
  ...
  -- تسجيل العملية في جدول balance_transactions كتحويل نقاط لرصيد
  BEGIN
    INSERT INTO public.balance_transactions (
      user_id, amount, type, method, provider_number, recipient_name, status, admin_notes, transaction_id
    ) VALUES (
      v_user_id, v_cash_value, 'deposit', 'points_conversion', 'system_points', 'تحويل نقاط', 'approved', 'تحويل نقاط إلى رصيد بالمحفظة', 'PTS_CONV_' || (EXTRACT(EPOCH FROM now())::bigint)
    );
  EXCEPTION WHEN OTHERS THEN
    -- التغاضي عن أي خطأ قد يحدث في إدراج السجل لعدم تعطيل عملية تحويل النقاط الأساسية
  END;
