...
export default function AdminReportsPage() {
  ...
  const [activeReportTab, setActiveReportTab] = useState<"places" | "suggestions" | "bugs" | "metro" | "monorail" | "lrt" | "railways" | "parking" | "contacts" | "microbus">("places");
  ...
  const isLrtFeedback = (f: any) =>
    !isParkingFeedback(f) &&
    (
      f.category === "القطار الكهربائي LRT" ||
      f.category === "lrt" ||
      f.category === "القطار الكهربائي" ||
      f.category === "القطار الكهربي" ||
      (f.title && (f.title.toLowerCase().includes("lrt") || f.title.toLowerCase().includes("القطار الكهربائي") || f.title.toLowerCase().includes("القطار الكهربي"))) ||
      (f.content && (f.content.toLowerCase().includes("lrt") || f.content.toLowerCase().includes("القطار الكهربائي") || f.content.toLowerCase().includes("القطار الكهربي")))
    );
  ...
  const isRailwayFeedback = (f: any) =>
    !isParkingFeedback(f) &&
    !isMonorailFeedback(f) &&
    !isLrtFeedback(f) &&
    (
      f.category === "سكك حديد مصر" ||
      f.category === "سكك حديد" ||
      f.category === "سكة حديد" ||
      f.category === "railways" ||
      (f.title && (
        f.title.toLowerCase().includes("سكك حديد") ||
        f.title.toLowerCase().includes("سكة حديد") ||
        f.title.toLowerCase().includes("خط قطار") ||
        f.title.toLowerCase().includes("محطة قطار") ||
        f.title.toLowerCase().includes("قطارات") ||
        f.title.toLowerCase().includes("تالجو") ||
        f.title.toLowerCase().includes("railway")
      )) ||
      (f.content && (
        f.content.toLowerCase().includes("سكك حديد") ||
        f.content.toLowerCase().includes("سكة حديد") ||
        f.content.toLowerCase().includes("قطارات مصر") ||
        f.content.toLowerCase().includes("قطار تالجو")
      ))
    );
  ...
  const monorailReports = appFeedbacks.filter(isMonorailFeedback);
  const filteredMonorailReports = monorailReports.filter(f => {
    return appStatusFilter === "all" || f.status === appStatusFilter;
  });

  const lrtReports = appFeedbacks.filter(isLrtFeedback);
  const filteredLrtReports = lrtReports.filter(f => {
    return appStatusFilter === "all" || f.status === appStatusFilter;
  });

  const railwayReports = appFeedbacks.filter(isRailwayFeedback);
  const filteredRailwayReports = railwayReports.filter(f => {
    return appStatusFilter === "all" || f.status === appStatusFilter;
  });
  ...
  const bugs = appFeedbacks.filter(f => f.type === "bug" && !isMetroFeedback(f) && !isMonorailFeedback(f) && !isLrtFeedback(f) && !isRailwayFeedback(f) && !isParkingFeedback(f));
  ...
  return (
    <div style={{ padding: "30px 0 120px 0" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "28px" }}>
        ...
        <button
          onClick={() => {
            ...
            else if (activeReportTab === "suggestions" || activeReportTab === "bugs" || activeReportTab === "metro" || activeReportTab === "monorail" || activeReportTab === "lrt" || activeReportTab === "railways" || activeReportTab === "parking") fetchAppFeedbacks();
      ...
      <div style={{ display: "flex", background: "rgba(255,255,255,0.03)", padding: "4px", borderRadius: "14px", border: "1px solid var(--border-glass)", marginBottom: "24px", maxWidth: "750px", overflowX: "auto" }}>
        ...
        >
          LRT ({lrtReports.length})
        </button>
        <button
          onClick={() => {
            setActiveReportTab("railways");
            setActiveReportId(null);
            setReplyText("");
            setActionStatus("");
            setAppStatusFilter("all");
        ...
        >
          سكك حديد مصر ({railwayReports.length})
        </button>
        <button
          onClick={() => {
        ...
        <>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", flexWrap: "wrap", gap: "12px" }}>
            ...
            <div style={{
              ...
              <i className="bx bx-check-double" style={{ fontSize: "1.2rem" }}></i>
              <span>عدد بلاغات المونوريل المحلولة: {monorailReports.filter(b => b.status === "action_taken").length}</span>
            </div>
          </div>

          {/* Filter Tabs for Monorail Reports Status */}
          <div style={{ display: "flex", gap: "10px", marginBottom: "20px", overflowX: "auto", paddingBottom: "8px" }}>
            {["all", "pending", "reviewed", "action_taken"].map((status) => {
              const count = status === "all" ? monorailReports.length : monorailReports.filter(r => r.status === status).length;
              let label = "الكل";
              if (status === "pending") label = "قيد النظر";
              if (status === "reviewed") label = "تمت المراجعة";
              if (status === "action_taken") label = "تم اتخاذ إجراء";
          ...
          {loadingAppFeedbacks ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "20px" }}>
              {filteredMonorailReports.map((feedback) => {
                const isOpen = activeReportId === feedback.id;
                            ...
                            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                              ...
                              <button
                                ...
                                onClick={() => setReportToDelete({
                                  id: feedback.id,
                                  type: "feedback",
                                  title: feedback.title ? `مشكلة مونوريل: ${feedback.title}` : "بلاغ مونوريل"
                                })}
                                disabled={updatingId !== null}
                                className="btn"
                                style={{
        ...
        <>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", flexWrap: "wrap", gap: "12px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              ...
              >
                <i className="bx bx-cog"></i>
                إدارة محطات القطار الكهربائي LRT
              </Link>
            </div>
            <div style={{
              background: "rgba(52, 199, 89, 0.12)",
              ...
              <i className="bx bx-check-double" style={{ fontSize: "1.2rem" }}></i>
              <span>عدد بلاغات LRT المحلولة: {lrtReports.filter(b => b.status === "action_taken").length}</span>
            </div>
          </div>

          {/* Filter Tabs for LRT Reports Status */}
          <div style={{ display: "flex", gap: "10px", marginBottom: "20px", overflowX: "auto", paddingBottom: "8px" }}>
            {["all", "pending", "reviewed", "action_taken"].map((status) => {
              const count = status === "all" ? lrtReports.length : lrtReports.filter(r => r.status === status).length;
              let label = "الكل";
              if (status === "pending") label = "قيد النظر";
              if (status === "reviewed") label = "تمت المراجعة";
              if (status === "action_taken") label = "تم اتخاذ إجراء";
          ...
          {loadingAppFeedbacks ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "20px" }}>
              {filteredLrtReports.map((feedback) => {
                const isOpen = activeReportId === feedback.id;
                            ...
                            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                              ...
                              <button
                                ...
                                onClick={() => setReportToDelete({
                                  id: feedback.id,
                                  type: "feedback",
                                  title: feedback.title ? `مشكلة LRT: ${feedback.title}` : "بلاغ LRT"
                                })}
                                disabled={updatingId !== null}
                                className="btn"
                                style={{
        ...
        <>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", flexWrap: "wrap", gap: "12px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              ...
              >
                <i className="bx bx-cog"></i>
                إدارة مواعيد وقطارات سكك حديد مصر
              </Link>
            </div>
            <div style={{
              background: "rgba(52, 199, 89, 0.12)",
              ...
              <i className="bx bx-check-double" style={{ fontSize: "1.2rem" }}></i>
              <span>عدد بلاغات سكك حديد مصر المحلولة: {railwayReports.filter(b => b.status === "action_taken").length}</span>
            </div>
          </div>

          {/* Filter Tabs for Railways Reports Status */}
          <div style={{ display: "flex", gap: "10px", marginBottom: "20px", overflowX: "auto", paddingBottom: "8px" }}>
            {["all", "pending", "reviewed", "action_taken"].map((status) => {
              const count = status === "all" ? railwayReports.length : railwayReports.filter(r => r.status === status).length;
              let label = "الكل";
              if (status === "pending") label = "قيد النظر";
              if (status === "reviewed") label = "تمت المراجعة";
              if (status === "action_taken") label = "تم اتخاذ إجراء";
          ...
          {loadingAppFeedbacks ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "20px" }}>
              {filteredRailwayReports.map((feedback) => {
                const isOpen = activeReportId === feedback.id;
                      ...
                      <div style={{ flex: 1, minWidth: "240px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "8px", flexWrap: "wrap" }}>
                          ...
                          <span style={{
                            ...
                            {feedback.category || "سكك حديد مصر"}
                          </span>
                          {getAppStatusBadge(feedback.status)}
                        </div>
                            ...
                            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                              ...
                              <button
                                ...
                                onClick={() => setReportToDelete({
                                  id: feedback.id,
                                  type: "feedback",
                                  title: feedback.title ? `مشكلة قطارات: ${feedback.title}` : "بلاغ سكك حديد مصر"
                                })}
                                disabled={updatingId !== null}
                                className="btn"
                                style={{
