...
/* ============================================================
   Cairo Metro Data — Lines 1, 2, 3 (with branch) & Future Lines
   ============================================================ */
...
function findRoute(
  ...
  if (!needsTransfer) {
    ...
    const transferDescs = transfers.map(t =>
      `انزل في محطة [${t.station}] وحوّل إلى ${LINE_NAMES[t.toLine]}`
    ).join("، ثم ");
...
export const METRO_PROBLEM_OPTIONS = [
  {
    id: "route_error",
...
export default function MetroPage() {
  ...
  const ACTIVE_METRO_TRIP_KEY = "cairo_metro_active_trip";
  ...
  // GSAP animation refs matching Railways layout
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      const sections = [sliderRef.current, searchPanelRef.current, detailsPanelRef.current, mapPanelRef.current, reportBannerRef.current].filter(Boolean);
  ...
  }, [explorerLine, line3ActiveBranch]);
    ...
    try {
      ...
      if (saved) {
        ...
        if (data && data.isTripActive && data.result) {
          // Keep active trip valid for 24 hours
          ...
          if (isFresh) {
            ...
            const stepIdx = typeof data.currentStepIndex === "number"
  ...
  }, [isTripRestored, isTripActive, currentStepIndex, result, selectedFrom, selectedTo, fromQuery, toQuery]);
  ...
  const getTicketPrice = (count: number): number => {
...
${reportDetails.trim()}`;
  ...
  const selectedLineObj = METRO_LINES_LIST.find(l => l.id === explorerLine) || METRO_LINES_LIST[0];
  ...
  return (
    ...
    <div className="main-container">
      ...
      <div className="container">
        ...
        >
          {METRO_LINES_LIST.map((line) => {
            const active = explorerLine === line.id;
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => {
                  setExplorerLine(line.id);
                  if (line.id === "line3") setLine3ActiveBranch("trunk");
                  if (detailsPanelRef.current) {
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px", position: "relative" }}>
            ...
            <div style={{ position: "relative", zIndex: showFromList ? 20 : 2 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px", flexWrap: "wrap", gap: "6px" }}>
                <label style={{ fontSize: "0.85rem", fontWeight: "700", color: "var(--text-secondary)", margin: 0, fontFamily: "var(--font-heading)" }}>
              ...
              <div style={{ position: "relative" }}>
                ...
                />
                {selectedFrom && (
                  <span style={{ position: "absolute", left: "12px", top: "50%", transform: "translateY(-50%)", fontSize: "0.72rem", background: "rgba(59, 130, 246, 0.15)", color: "var(--color-secondary)", padding: "2px 8px", borderRadius: "8px", fontWeight: "600" }}>تم الاختيار ✔</span>
                )}
              </div>
              {showFromList && filteredFrom.length > 0 && (
                <div style={{
                  ...
                  marginTop: "4px", boxShadow: "0 10px 25px rgba(0,0,0,0.2)"
                }}>
                  {filteredFrom.map(s => {
            ...
            <div style={{ position: "relative", zIndex: showToList ? 20 : 1 }}>
              ...
              {showToList && filteredTo.length > 0 && (
                <div style={{
                  ...
                  background: "var(--bg-secondary)", border: "1px solid var(--border-glass)",
                  borderRadius: "var(--radius-card)", overflow: "hidden", zIndex: 100, maxHeight: "220px", overflowY: "auto",
                  marginTop: "4px", boxShadow: "0 10px 25px rgba(0,0,0,0.2)", fontFamily: "var(--font-sub)"
                }}>
                  {filteredTo.map(s => {
            ...
            <div style={{ marginTop: "12px", display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              {!result.found ? (
                <div style={{ textAlign: "center", color: "var(--text-secondary)", padding: "20px 0" }}>
                  ...
                  <h4 style={{ fontWeight: "700", color: "var(--text-primary)", marginBottom: "6px" }}>عذراً، تعذر العثور على مسار</h4>
                ...
                <>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "10px" }}>
                    ...
                    <div style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-glass)", borderRadius: "var(--ra-8)", padding: "12px", textAlign: "center" }}>
                      <div style={{ fontSize: "1.2rem", fontWeight: "800", color: "var(--color-secondary)" }}>{result.stationCount}</div>
                      <div style={{ fontSize: "0.75rem", color: "var(--text-secondary)", fontWeight: "600", marginTop: "4px" }}>عدد المحطات</div>
                    </div>
                    {/* Price */}
                    <div style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-glass)", borderRadius: "var(--ra-8)", padding: "12px", textAlign: "center" }}>
                      <div style={{ fontSize: "1.2rem", fontWeight: "800", color: "var(--colorSuccess)" }}>{result.price} ج.م</div>
                      <div style={{ fontSize: "0.75rem", color: "var(--text-secondary)", fontWeight: "600", marginTop: "4px" }}>سعر التذكرة</div>
                    </div>
                    {/* Estimated Time */}
                    <div style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-glass)", borderRadius: "var(--ra-8)", padding: "12px", textAlign: "center" }}>
                      <div style={{ fontSize: "1.2rem", fontWeight: "800", color: "var(--color-secondary)" }}>{result.estimatedTime} د</div>
                      <div style={{ fontSize: "0.75rem", color: "var(--text-secondary)", fontWeight: "600", marginTop: "4px" }}>وقت الوصول</div>
                    </div>
                    {/* Number of Transfers */}
                    <div style={{ background: "var(--bg-secondary)", border: "1px solid var(--border-glass)", borderRadius: "var(--ra-8)", padding: "12px", textAlign: "center" }}>
                      ...
                      <div style={{ fontSize: "0.75rem", color: "var(--text-secondary)", fontWeight: "600", marginTop: "4px" }}>نوع الرحلة</div>
                    </div>
                  </div>
                  ...
                  {isTripActive && (
                    <div style={{
                      ...
                      <div style={{ fontSize: "0.92rem", fontWeight: "600", marginBottom: "6px", color: "var(--text-secondary)" }}>
                        أنت الآن في محطة: <strong style={{ color: "var(--text-primary)", fontSize: "1.05rem" }}>{result.detailedPath[currentStepIndex].station}</strong>
                      ...
                      {(() => {
                        ...
                        return (
                          <div style={{ fontSize: "0.85rem", fontWeight: "600", marginBottom: "14px", color: "var(--text-secondary)" }}>
                            الوقت المتبقي المقدر: <strong style={{ color: "var(--color-secondary)" }}>{remainingTime} دقيقة</strong>
                          </div>
                        );
                      })()}
                      ...
                      {result.detailedPath[currentStepIndex].isTransferPoint && (
                        <div style={{
                          ...
                          <strong style={{ color: LINE_COLORS[result.detailedPath[currentStepIndex].targetLine!] }}>
                      ...
                      {currentStepIndex < result.detailedPath.length - 1 ? (
                        ...
                        <div style={{
                          ...
                          <h4 style={{ color: "var(--colorSuccess)", fontWeight: "800", margin: "0 0 4px" }}>حمدلله على السلامة!</h4>
                          <p style={{ fontSize: "0.84rem", color: "var(--text-secondary)", margin: "0 0 10px" }}>لقد وصلت إلى وجهتك محطة {result.detailedPath[currentStepIndex].station}.</p>
                          <button
                            type="button"
                            onClick={() => {
                              setIsTripActive(false);
                              ...
                              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "16px", flexShrink: 0 }}>
                                {isPassed ? (
                                  ...
                                  <div style={{
                                    ...
                                    borderRadius: "50%",
                                    backgroundColor: isUnderConstruction ? "transparent" : (isFirst || isLast ? activeColor : (isTransfer ? "var(--colorWarning, #f59e0b)" : activeColor)),
                                    border: isUnderConstruction ? `2px dashed var(--colorDanger)` : (isFirst || isLast ? `2px solid var(--bgPrimary)` : "none"),
                                    boxShadow: isUnderConstruction ? "none" : (isFirst || isLast ? `0 0 0 2px ${activeColor}` : "none")
                                  }} />
                                )}
                              </div>

                              {/* Text & Badges */}
                              <div style={{ display: "flex", alignItems: "center", gap: "8px", flexGrow: 1, opacity: isPassed ? 0.5 : 1 }}>
                                <span style={{
                                  fontSize: "0.88rem",
                                  fontWeight: isFirst || isLast || isTransfer || isCurrent ? "700" : "500",
                                  color: isUnderConstruction ? "#ef4444" : (isCurrent ? "var(--color-secondary)" : (isFirst || isLast ? "var(--text-primary)" : "var(--text-secondary)")),
                                  display: "inline-flex",
                                  alignItems: "center",
                                  gap: "8px"
                                }}>
                                  {node.station}
                                  {isFirst && <span style={{ fontSize: "0.72rem", color: "var(--text-muted)" }}>(محطة الركوب)</span>}
                                  {isLast && <span style={{ fontSize: "0.72rem", color: "var(--text-muted)" }}>(محطة الوصول)</span>}
                                </span>
        ...
        {/* Main Details Panel for Selected Line */}
        <div ref={detailsPanelRef} className="details-panel">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px" }}>
            ...
            {explorerLine === "line3" && (
              <div className="tabs" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "6px", marginBottom: 0, padding: "4px" }}>
                {[
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "12px", minHeight: "34px" }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "16px", flexShrink: 0 }}>
                          <div style={{
                            ...
                            borderRadius: "50%",
                            backgroundColor: isUnderConstruction ? "transparent" : (isTransfer ? "var(--colorWarning, #f59e0b)" : color),
                            border: isUnderConstruction ? `2px dashed var(--colorDanger)` : (isFirst || isLast ? `2px solid var(--bgPrimary)` : "none"),
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "8px", flexGrow: 1, flexWrap: "wrap" }}>
                          <span
                            ...
                            style={{
                              ...
                              color: isUnderConstruction ? "#ef4444" : (isFirst || isLast ? "var(--text-primary)" : "var(--text-primary)"),
                              display: "inline-flex",
                              alignItems: "center",
                              gap: "6px",
                              cursor: "pointer"
        ...
        {/* Report Problem Banner Matching Railways */}
              ...
              {reportSuccess ? (
                ...
                <form onSubmit={handleSubmitReport} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                  ...
                  <div>
                    ...
                    <div className="tabs" style={{ display: "grid", gridTemplateColumns: (selectedFrom && selectedTo) ? "repeat(3, 1fr)" : "repeat(2, 1fr)", gap: "6px" }}>
                      <button
                        type="button"
                        onClick={() => {
                          setReportTargetScope("general");
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                                    ...
                                    {st.isTransfer && (
                                      <span style={{ fontSize: "0.68rem", background: "rgba(59, 130, 246, 0.15)", color: "var(--color-secondary)", padding: "1px 6px", borderRadius: "4px", fontWeight: "600" }}>
                                        تبادلية
                                      </span>
                                    )}
                                    {isSelected && (
                  ...
                  <div style={{ position: "relative" }}>
                    ...
                    {(() => {
                      const curOpt = METRO_PROBLEM_OPTIONS.find(o => o.id === reportProblemType) || METRO_PROBLEM_OPTIONS[0];
                      return (
                        <button
                          type="button"
