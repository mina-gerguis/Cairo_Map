...
import { Place, PlaceCategory, initialPlaces, FEATURES_LIST, CATEGORIES_STRUCTURE, formatBoxIcon, normalizePlaceCategory, getMainCategoryImage } from "@/data/places";
...
function HomeContent() {
  ...
  const [authModalMessage, setAuthModalMessage] = useState("يرجى تسجيل الدخول أولاً لإضافة الأماكن المفضلة.");
  ...
  const toggleFavorite = async (e: React.MouseEvent, placeId: string) => {
    e.stopPropagation();
    if (!user || !supabase) {
      setAuthModalMessage("يرجى تسجيل الدخول أولاً لإضافة الأماكن المفضلة.");
  ...
  /* Geolocation */
  const handleLocationSuccess = (lat: number, lng: number) => {
    ...
    <>
      ...
      <section className="hero-section">
        ...
        <div className="hero-content">
                      ...
                      <div className="suggestion-item-main" style={{ display: "flex", alignItems: "center", gap: "14px", flex: 1, minWidth: 0 }}>
                        ...
                        <div className="suggestion-details" style={{ display: "flex", flexDirection: "column", textAlign: "right", minWidth: 0, flex: 1 }}>
                          <span className="suggestion-name" style={{ fontFamily: "var(--font-cairo), sans-serif", fontWeight: 700, fontSize: "0.98rem", color: isLight ? "#1f2937" : "#f3f4f6", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                            {place.name}
                            {place.name_en && (
                              <span style={{ fontSize: "0.8rem", color: isLight ? "#6b7280" : "#9ca3af", marginRight: "6px", fontWeight: "normal" }}>
                                ({place.name_en})
                              </span>
                            )}
                          </span>
          ...
          <div className="hero-actions" style={{ zIndex: "10" }}>
            <button className="hero-btn-primary sub-title" onClick={() => document.getElementById('places-section')?.scrollIntoView({ behavior: 'smooth' })} >
            ...
            >
              {locationLoading ? (
                <span className="btn-secondary" style={{ width: "16px", height: "16px", borderRadius: "50%", display: "inline-block", animation: "spin 0.8s linear infinite", marginLeft: "8px" }} />
              ) : (
          ...
          <div className="hero-stats">
            <div className="hero-stat">
              <div className="hero-stat-number sub-title">{places.length}+</div>
      ...
      <div className="app-container" id="places-section">
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "12px", marginBottom: "20px", flexWrap: "wrap" }}>
          ...
          <div className="sub-title" style={{ display: "flex", gap: "8px", overflowX: "auto", paddingBottom: "4px", WebkitOverflowScrolling: "touch", scrollbarWidth: "none", flexGrow: 1 }}>
                  ...
                  <>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "10px", marginBottom: "20px" }}>
                      ...
                      >
                        ...
                        <span style={{ fontSize: "0.75rem", fontWeight: "500" }}>المفضلة</span>
                      </button>
                    </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginTop: "24px", marginBottom: "20px" }}>
                      ...
                      <div style={{ display: "flex", gap: "10px" }}>
                        <button
                          onClick={() => {
                            if (!user) {
                              setAuthModalMessage("يرجى تسجيل الدخول أولاً لإضافة ملاحظة.");
                              setShowAuthModal(true);
                              return;
                            }
          ...
          <div className="ios-sheet" onClick={(e: any) => e.stopPropagation()}>
            ...
            <div className="ios-sheet-content">
              ...
              <form onSubmit={handleAddPlace} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                <input className="input-fields" style={{ paddingRight: "16px" }} placeholder="اسم المكان *" value={newName} onChange={(e) => setNewName(e.target.value)} required />
                <select className="input-fields help-select" style={{ paddingRight: "16px" }} value={newCategory} onChange={(e) => setNewCategory(e.target.value as PlaceCategory)}>
                ...
                <input className="input-fields" style={{ paddingRight: "16px" }} placeholder="الموقع المختصر (مثال: مصر الجديدة) *" value={newBriefLocation} onChange={(e) => setNewBriefLocation(e.target.value)} required />
                <input className="input-fields" style={{ paddingRight: "16px" }} placeholder="العنوان التفصيلي" value={newFullAddress} onChange={(e) => setNewFullAddress(e.target.value)} />
                <input className="input-fields" style={{ paddingRight: "16px" }} placeholder="أرقام التليفون (افصل بفاصلة)" value={newPhones} onChange={(e) => setNewPhones(e.target.value)} />
                <input className="input-fields" style={{ paddingRight: "16px" }} placeholder="رابط خرائط جوجل" value={newMapsUrl} onChange={(e) => setNewMapsUrl(e.target.value)} />
                <input className="input-fields" style={{ paddingRight: "16px" }} placeholder="رابط صورة المكان" value={newImageUrl} onChange={(e) => setNewImageUrl(e.target.value)} />
                <input className="input-fields" style={{ paddingRight: "16px" }} placeholder="مواعيد العمل" value={newWorkingHours} onChange={(e) => setNewWorkingHours(e.target.value)} />
                <textarea className="input-fields" style={{ paddingRight: "16px", minHeight: "80px", resize: "vertical" }} placeholder="وصف المكان" value={newDescription} onChange={(e) => setNewDescription(e.target.value)} />
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                  <input className="input-fields" style={{ paddingRight: "16px" }} placeholder="خط العرض (Lat)" value={newLat} onChange={(e) => setNewLat(e.target.value)} type="number" step="any" />
                  <input className="input-fields" style={{ paddingRight: "16px" }} placeholder="خط الطول (Lng)" value={newLng} onChange={(e) => setNewLng(e.target.value)} type="number" step="any" />
                </div>
                <div style={{ display: "flex", gap: "10px" }}>
                  <button type="submit" className="btn btn-primary" style={{ flex: 1 }}>إضافة المكان</button>
                  <button type="button" className="btn" onClick={() => setShowAddModal(false)} style={{ flex: 1 }}><i className="bx bx-x" style={{ fontSize: "1.2rem" }}></i><i className="bx bx-x" style={{ fontSize: "1.2rem" }}></i> إلغاء</button>
                </div>
              </form>
            </div>
          </div>
