...
const fallbackPlans = [
  {
    ...
    price_monthly: 0,
    price_yearly: 0,
    features: ["تصفح خطوط المترو الأساسية والبحث", "عرض جداول المواعيد والمحطات التبادلية", "تصفح الأماكن العامة"]
  },
  {
    id: "mishwar",
    name: "باقة المشوار",
...
export default function ProfilePage() {
  ...
  const [deviceToDeactivate, setDeviceToDeactivate] = useState<{ deviceId: string; sessionId: string; isCurrentDevice: boolean } | null>(null);
  ...
  const executeDeactivateDevice = async (deviceId: string, sessionId: string, isCurrentDevice: boolean) => {
    if (!supabase) return;
    try {
  ...
  // Check on mount and on location change to expand help and support section
  useEffect(() => {
    if (typeof window === "undefined") return;
  ...
  const getPlan = (planId: string) => {
    return dbPlans.find((p: any) => p.id === planId) || fallbackPlans.find(p => p.id === planId)!;
  };

  const getPlanPrice = (planId: string, period: "daily" | "monthly" | "yearly" | null) => {
    if (planId === "free") return 0;

    const dbPlan = getPlan(planId);
    let basePrice = 0;
    if (period === "daily") {
      basePrice = Number(dbPlan.price_daily);
    } else if (period === "monthly") {
      basePrice = Number(dbPlan.price_monthly);
    } else if (period === "yearly") {
      basePrice = Number(dbPlan.price_yearly);
    }
    ...
    if (hasActiveSilver && planId === "gold") {
      const silverPlan = getPlan("silver");
    ...
    if (hasActiveMishwar && (planId === "silver" || planId === "gold")) {
      const mishwarPlan = getPlan("mishwar");
  ...
  const handleConvertPoints = async () => {
    ...
    try {
      ...
      if (data && data.success) {
        ...
        // Log points conversion transaction for financial stats auditing
        try {
  ...
  const handleEnrollTOTP = async () => {
    ...
    setMfaError("");
    try {
      // Clean up any existing unverified factors first to avoid duplicate name/factor error
  ...
  return (
    <div className={styles.container}>
        ...
        <CustomModal
          ...
          iconNode={
            <div style={{
              ...
              <i className={`bx ${message.type === 'error' ? 'bx-error-circle' : 'bx-check-circle'}`} style={{ fontSize: "2.2rem" }}></i>
            </div>
          }
            ...
            {editMode ? (
              <div className={styles.formGap}>
                ...
                <div>
                  ...
                  <input type="text" className="input-fields" value={formData.fullName} onChange={e => setFormData({ ...formData, fullName: e.target.value })} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    className={`input-fields ${styles.usernameInput} ${(formData.username.length > 0 && formData.username.length < 3) ? styles.usernameInputInvalid : ''}`}
                ...
                <div>
                  ...
                  <input type="email" className={`input-fields ${styles.emailInput}`} value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} />
                </div>
                ...
                <div>
                  ...
                  <div className={styles.phoneInputContainer}>
                    ...
                    <input type="tel" className={`input-fields ${styles.phoneInput}`} value={formData.phone} onChange={e => {
                ...
                <div>
                  ...
                  <select className="input-fields help-select" value={formData.governorate} onChange={(e) => setFormData({ ...formData, governorate: e.target.value, city: "" })}>
                ...
                {formData.governorate && (
                  <div>
                    ...
                    <select className="input-fields help-select" value={formData.city} onChange={e => setFormData({ ...formData, city: e.target.value })}>
              ...
              <div className={styles.formGap}>
                  ...
                  {profile?.interests && profile.interests.length > 0 ? (
                    <div className={styles.interestsGrid}>
                      {profile.interests.map((intId: string) => {
                        ...
                        return (
                          <div key={intId} className={`category-pill active sub-title ${styles.interestPillReadonly}`}>
                            <i className={`${interest.icon} ${styles.interestPillIcon}`} /> {interest.label}
                          </div>
                        );
                      })}
                    ...
                    <div className={styles.noInterestsCard}>
                      ...
                      {isOwnProfile && (
                        <button className={`btn btn-secondary ${styles.addInterestsBtn}`} onClick={(e) => { e.stopPropagation(); setEditMode(true); }}>
                          أضف الآن
                        </button>
                      )}
                    </div>
                ...
                {isOwnProfile && (
                  <div className={styles.formButtonsRow}>
                    <button className={`btn btn-primary ${styles.flex1}`} onClick={() => setEditMode(true)} style={{ fontSize: "14px", fontWeight: "normal" }}>
                      تعديل البيانات
                    </button>
                  </div>
                )}
      ...
      {user && (
        <div className={`glass-panel ${styles.userBadgesContainer}`} style={{ justifyContent: "center", marginBottom: "24px", marginTop: "8px", fontFamily: "var(--font-heading)" }}>
          {/* Points Badge */}
          <div
          ...
          {/* Promo Balance Badge */}
          <div className={`sub-title ${styles.badgePill} ${styles.badgePromo}`} title="الرصيد الترويجي">
      ...
      {/* ─── Security & 2FA (For logged in users) ─── */}
      {user && isOwnProfile && (
        <div className={styles.sectionCard}>
      ...
      {/* ─── Suggestions & Bug Reports (For logged in users) ─── */}
      {user && isOwnProfile && (
        <div className={styles.sectionCard}>
            ...
            <div onClick={(e) => e.stopPropagation()} className={styles.helpExpandedContent}>
                ...
                <div className={styles.faqList}>
                  ...
                  {profile?.is_admin && (
                    <form onSubmit={handleAddFAQ} className={styles.adminFaqForm}>
                      ...
                      <input
                        required className="input-fields" placeholder="السؤال..."
                        value={faqQuestion} onChange={e => setFaqQuestion(e.target.value)}
                      />
                      <textarea
                        required className={`input-fields ${styles.adminFaqTextarea}`} placeholder="الإجابة..."
                        value={faqAnswer} onChange={e => setFaqAnswer(e.target.value)}
                      />
                      <button type="submit" disabled={faqLoading} className={`btn btn-primary ${styles.adminFaqSubmitBtn}`}>
              ...
              {helpTab === "social" && (
                <div className={styles.socialGrid}>
                  <a href="https://www.whatsapp.com/channel/0029VbE5UzFGpLHIjyTobP3p" target="_blank" rel="noopener noreferrer" className={`category-pill ${styles.socialPill}`}>
                    <i className={`bx bxl-whatsapp ${styles.socialIconWhatsapp}`}></i>
                    <span>قناة الواتساب</span>
                  </a>
                  <a href="https://www.facebook.com/share/1LTxiYMaJm/?mibextid=wwXIfr" target="_blank" rel="noopener noreferrer" className={`category-pill ${styles.socialPill}`}>
                    <i className={`bx bxl-facebook-circle ${styles.socialIconFacebook}`}></i>
                    <span>فيسبوك</span>
                  </a>
                  <a href="https://x.com/cairo_map" target="_blank" rel="noopener noreferrer" className={`category-pill ${styles.socialPill}`}>
                    <svg width="20px" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                    </svg>
                    <span>تويتر</span>
                  </a>
                  <a href="https://instagram.com/map_cairo" target="_blank" rel="noopener noreferrer" className={`category-pill ${styles.socialPill}`}>
                ...
                <div>
                  {contactSubmitted ? (
                    <div className={styles.contactSuccess}>
                      ...
                      <button className="btn" onClick={() => setContactSubmitted(false)}>إرسال رسالة أخرى</button>
                    </div>
                  ) : (
                    <form onSubmit={handleContactSubmit} className={styles.contactForm}>
                      <div className={styles.grid2Col}>
                        <input
                          required className="input-fields" placeholder="الاسم الأول"
                          value={contactForm.firstName} onChange={e => setContactForm({ ...contactForm, firstName: e.target.value })}
                        />
                        <input
                          required className="input-fields" placeholder="الاسم الأخير"
                          value={contactForm.lastName} onChange={e => setContactForm({ ...contactForm, lastName: e.target.value })}
                        />
                      </div>
                      <div className={styles.grid2Col}>
                        <input
                          required className={`input-fields ${styles.inputLtrRight}`} placeholder="رقم الهاتف"
                          value={contactForm.phone} onChange={e => setContactForm({ ...contactForm, phone: e.target.value })}
                        />
                        <input
                          required className={`input-fields ${styles.inputLtrRight}`} type="email" placeholder="البريد الإلكتروني"
                          value={contactForm.email} onChange={e => setContactForm({ ...contactForm, email: e.target.value })}
                        />
                      </div>
                      <select
                        required className="input-fields help-select"
                      ...
                      <textarea
                        required className={`input-fields ${styles.contactTextarea}`} placeholder="اكتب تفاصيل رسالتك هنا..."
                        value={contactForm.message} onChange={e => setContactForm({ ...contactForm, message: e.target.value })}
                      />
                      <button type="submit" disabled={contactLoading} className={`btn btn-primary ${styles.contactSubmitBtn}`}>
      ...
      <div className={styles.footerCard} style={{ marginBottom: "100px" }}>
        ...
        <div className={styles.socialRow}>
          <a href="https://www.facebook.com/share/1LTxiYMaJm/?mibextid=wwXIfr" target="_blank" rel="noopener noreferrer" className={styles.socialIconWrapper} title="Facebook">
            <svg viewBox="0 0 24 24" fill="currentColor">
              <path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z" />
            </svg>
          </a>
          <a href="https://www.instagram.com/map_cairo/" target="_blank" rel="noopener noreferrer" className={styles.socialIconWrapper} title="Instagram">
            <svg viewBox="0 0 24 24" fill="currentColor">
          ...
          <a href="https://www.linkedin.com/showcase/cairo-map" target="_blank" rel="noopener noreferrer" className={styles.socialIconWrapper} title="LinkedIn">
            <svg viewBox="0 0 24 24" fill="currentColor">
              <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
            </svg>
          </a>
          <a href="https://x.com/cairo_map" target="_blank" rel="noopener noreferrer" className={styles.socialIconWrapper} title="X (Twitter)">
            <svg viewBox="0 0 24 24" fill="currentColor">
              <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
            </svg>
          </a>
        ...
        <div className={styles.copyrightText}>
          © 2026 ماب القاهرة. جميع الحقوق محفوظة
        </div>
      </div>
      ...
      >
        <div style={{ textAlign: "right" }}>
          ...
          {mfaStep === "selection" && (
            <>
              <p className={styles.mfaStepText} style={{ textAlign: "center", marginBottom: "20px" }}>
      ...
      <div>
          ...
          <div style={{ textAlign: "right" }}>
            <div className={styles.devicesListContainer}>
              {loadingDevices ? (
                ...
                <p className={styles.noDevicesText} style={{ textAlign: "center" }}>لا توجد أجهزة مسجلة حالياً.</p>
              ) : (
                        ...
                        <div className={styles.deviceItemLeft}>
                          ...
                          <div className={styles.deviceInfoTexts}>
                            ...
                            <div className={styles.deviceMetaRow}>
                              ...
                              <span style={{ fontFamily: "var(--font-heading)" }}>📅 {new Date(device.logged_in_at).toLocaleDateString("ar-EG", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</span>
                            </div>
        ...
        {/* Device Deactivation Confirmation Modal */}
        <CustomModal
          isOpen={deviceToDeactivate !== null}
          onClose={() => setDeviceToDeactivate(null)}
          title={deviceToDeactivate?.isCurrentDevice ? "تسجيل الخروج من الجهاز الحالي" : "إنهاء جلسة الجهاز"}
          message={
          ...
          primaryButton={{
            ...
            onClick: () => {
              if (deviceToDeactivate) {
                executeDeactivateDevice(deviceToDeactivate.deviceId, deviceToDeactivate.sessionId, deviceToDeactivate.isCurrentDevice);
      ...
      <CustomModal
        ...
        onClose={() => { setShowPasswordModal(false); setPasswordForm({ new: "", confirm: "" }); }}
      ...
      <CustomModal
        ...
        message={hasRemindersAccess ? `إجمالي الملاحظات والتذكيرات المضافة للأماكن: ${reminders.length}` : undefined}
        ...
        <div className={`modal-backdrop ${styles.modalBackdropSlow}`} onClick={() => setShowPointsModal(false)}>
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "20px", borderTop: "1px solid var(--borderGlass)", paddingTop: "20px" }}>
              ...
              {/* How to Earn */}
              <div>
                <h4 className="sub-title" style={{ margin: "0 0 10px", fontSize: "0.92rem", fontWeight: "800", color: "#002a9eff", display: "flex", alignItems: "center", gap: "6px" }}>
              ...
              {/* How to Use */}
              <div>
                <h4 className="sub-title" style={{ margin: "0 0 10px", fontSize: "0.92rem", fontWeight: "800", color: "#10b981", display: "flex", alignItems: "center", gap: "6px" }}>
