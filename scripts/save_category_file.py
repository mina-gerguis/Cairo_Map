...
def populate_sheet_data(ws, places_list, category_name):
...
def create_single_category_file(places_list, category_name, filename_ar):
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = category_name[:31]
    populate_sheet_data(ws, places_list, category_name)
    ...
    ws_guide.append([category_name, places_list[0].get("sub_categories", "") if places_list else ""])
    for col_idx in [1, 2]:
...
print("Category generator core module ready.")
